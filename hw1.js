...
const Product = class Product {
    ...
    constructor(products) {
        var newProduct = products.split(',');
        this.name = String(newProduct[0]);
        this.price = Number(newProduct[1]);
        this.availability = String(newProduct[2]);
    }
    ...
    static halfOff = (products) => {
        ...
        // Deep copy to separate object
        var solution = JSON.parse(JSON.stringify(products));

        solution.forEach(prod => `${prod.price = prod.price * 0.5}`);
    ...
    static printProducts = (products) => {
        products.forEach(prod => console.log( `Product: ${prod.name}, Cost: $${prod.price.toFixed(2)}, Availability: ${prod.availability == "In Stock" ? "Yes" : "No"}`));
    }
